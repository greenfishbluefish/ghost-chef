describe Route53 do
  ...
  def build_list_hosted_zones_by_name(names)
  ...
  context '#zone_for_name' do
    ...
    context 'with one zone found' do
      before(:each) { build_list_hosted_zones_by_name(['foo.com']) }
  ...
  context '#ensure_dns_for_s3' do
    context 'with no zones found' do
      ...
      it "returns false" do
        ...
        expect(Route53.ensure_dns_for_s3('foo.com')).to be false
      end
    end

    context 'with the zone found' do
      before(:each) {
        build_list_hosted_zones_by_name(['foo.com'])
				expect(client).to receive(:list_hosted_zones_by_name)
					.with(
						dns_name: 'foo.com',
            max_items: 1,
